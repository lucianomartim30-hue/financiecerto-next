...
import { formatBRL, parcelaPrice, calcularSeguros, TAXA_MCMV_ANUAL, TAXA_SBPE_ANUAL, TETO_MCMV } from '@/lib/calculos';
...
function NaPlantaContent() {
  ...
  // ─── FIX 1: Entrada total + Ato → Mensais é derivado automaticamente ─────────
  const [entradaPerc, setEntradaPerc] = useState(20); // total % pago à construtora durante obra
  const [atoPerc,     setAtoPerc]     = useState(5);  // subconjunto do ato (pago na assinatura)
  ...
  // ─── FIX 1: Mensais = Entrada - Ato (automático) ──────────────────────────
  const atoPercValido    = Math.min(atoPerc, entradaPerc); // ato nunca pode superar entrada
  const mensaisPercAuto  = entradaPerc - atoPercValido;    // o que sobra do ato vira mensais
  ...
  const totalPercConstrutora = entradaPerc + sinaisEfetivo + anuaisEfetivo + chavesEfetivo;
  const financiadoPerc       = Math.max(0, 100 - totalPercConstrutora);
  const excessoPerc          = Math.max(0, totalPercConstrutora - 100);

  // Valores absolutos
  const ato          = valorImovel * atoPercValido / 100;
  const totalMensais = valorImovel * mensaisPercAuto / 100;
  ...
  // ─── FIX 2 & 3: Juros evolutivos MCMV incluídos no limite de 30% ──────────
  const parcelaFinanc      = parcelaPrice(valorFinanciado, taxa, 35 * 12);
  const seguros            = calcularSeguros(valorFinanciado);
  const COEF_MEDIO_OBRA    = 0.655; // custo médio da obra (curva SIOPI/Caixa)
  const jurosEvolutivos    = isMCMV && valorFinanciado > 0
    ? Math.round(parcelaPrice(valorFinanciado, TAXA_MCMV_ANUAL, 35 * 12) * COEF_MEDIO_OBRA + seguros.total)
    : 0;

  const burdenMensalTotal = mensalValor + jurosEvolutivos; // custo mensal real durante a obra
  const limite30          = rendaNum * 0.30;
  const dentro30          = burdenMensalTotal <= limite30 || mensalValor === 0;
  ...
  const valido = valorImovel > 0 && rendaNum > 0 && totalPercConstrutora <= 100;

  // Garante que ato nunca ultrapassa entrada no estado
  function handleAtoChange(v: number) {
    setAtoPerc(Math.min(v, entradaPerc));
  }
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <select value={prazoObras} onChange={e => setPrazoObras(Number(e.target.value))} style={selectStyle}>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#78716c', marginBottom: '8px' }}>
              Escolha a estrutura que a construtora oferece.
            </p>
          ...
          {/* ─── FIX 1: Entrada total + Ato → Mensais derivado automaticamente ── */}
          <div style={{ background: '#fafaf9', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
            <p style={{ fontSize: '13px', fontWeight: '700', color: '#1c1917', marginBottom: '12px' }}>
              Entrada durante a obra
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <PercSelect
                label="Total da entrada"
                hint="% do valor pago à construtora"
                valor={entradaPerc}
                onChange={v => { setEntradaPerc(v); if (atoPerc > v) setAtoPerc(v); }}
                options={[10, 15, 20, 25, 30]}
                base={valorImovel}
              />
              <PercSelect
                label="Ato (na assinatura)"
                hint="Pago no dia da assinatura"
                valor={atoPercValido}
                onChange={handleAtoChange}
                options={[3, 5, 8, 10, 12, 15].filter(o => o <= entradaPerc)}
                base={valorImovel}
              />
            </div>

            {/* Mensais derivado — FIX 1 */}
            <div style={{
              ...
              <div>
                <p style={{ fontSize: '13px', fontWeight: '600', color: '#1c1917', marginBottom: '2px' }}>
                  📅 Parcelas mensais durante a obra
                </p>
                <p style={{ fontSize: '12px', color: '#78716c' }}>
                  Após o ato de {formatBRL(Math.round(ato))}, os restantes{' '}
                  <strong style={{ color: '#1c1917' }}>{formatBRL(Math.round(totalMensais))}</strong> ({mensaisPercAuto}% do valor)
                  {' '}são divididos em <strong style={{ color: '#1c1917' }}>{prazoObras} mensais de{' '}
                  {formatBRL(Math.round(mensalValor))}/mês</strong>
                </p>
              </div>
            </div>
          </div>

          {/* Sinais, Anuais, Chaves (modos extras) */}
          {(usaSinais || usaAnuais || usaChaves) && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              {usaSinais && (
                <>
                  ...
                  />
                  <div>
                    <p style={{ fontSize: '12px', fontWeight: '600', color: '#78716c', marginBottom: '2px' }}>FREQUÊNCIA</p>
                    <p style={{ fontSize: '11px', color: '#a8a29e', marginBottom: '4px' }}>Intervalo entre sinais</p>
              ...
              {usaAnuais && (
                <>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '11px', color: '#a8a29e', marginBottom: '4px' }}>Parcelas por ano</p>
          ...
          <div style={{
            ...
            <div>
              <p style={{ fontSize: '13px', fontWeight: '600', color: excessoPerc > 0 ? '#dc2626' : '#15803d', marginBottom: '2px' }}>
                {excessoPerc > 0 ? `⚠️ Total ultrapassa 100% em ${excessoPerc}%` : '🏦 Financiamento bancário (automático)'}
              </p>
              <p style={{ fontSize: '12px', color: '#78716c' }}>
                {excessoPerc > 0
                  ? 'Reduza os percentuais para fechar o cálculo.'
                  : `${financiadoPerc.toFixed(0)}% do valor = ${valorImovel > 0 ? formatBRL(Math.round(valorFinanciado)) : '—'}`}
              </p>
            </div>
        ...
        {valido && (
          <div style={{
            ...
            <BlocoFluxo emoji="📋" titulo="Na Assinatura" cor="#44403c">
              <LinhaFluxo label={`Ato — ${atoPercValido}% do valor`} valor={formatBRL(Math.round(ato))} destaque />
              <Saldo
                label="Saldo restante à construtora após o ato"
                valor={formatBRL(Math.round(saldoAposAto))}
            ...
            <BlocoFluxo emoji="🏗️" titulo={`Durante a Obra — ${prazoObras} meses`} cor="#d97706">
              ...
              <LinhaFluxo
                label={`Mensais × ${prazoObras} (${mensaisPercAuto}% do valor)`}
                valor={`${formatBRL(Math.round(mensalValor))}/mês`}
                sublabel={`Total: ${formatBRL(Math.round(totalMensais))}`}
              />
              ...
              {usaSinais && sinalValor > 0 && (
                <LinhaFluxo
                  ...
                  sublabel={`Total: ${formatBRL(Math.round(totalSinais))}`}
                />
              )}
              ...
              {usaAnuais && anualValor > 0 && (
                <LinhaFluxo
                  ...
                  sublabel={`Total: ${formatBRL(Math.round(totalAnuais))}`}
                />
              )}

              {/* FIX 2: Juros evolutivos MCMV */}
              {isMCMV && jurosEvolutivos > 0 && (
                <LinhaFluxo
                  label="Juros evolutivos ao banco (MCMV Crédito Associativo)"
                  valor={`~${formatBRL(jurosEvolutivos)}/mês`}
                  sublabel="Crescem conforme o progresso da obra (média estimada)"
                />
              )}

              {/* FIX 3: Alerta 30% com juros evolutivos incluídos */}
              {mensalValor > 0 && (
                <div style={{
                  ...
                  <div>
                    <p style={{ fontSize: '12px', fontWeight: '600', color: dentro30 ? '#15803d' : '#dc2626', marginBottom: '2px' }}>
                      {dentro30
                        ? 'Comprometimento mensal dentro do limite (30% da renda)'
                        : 'Comprometimento mensal acima do limite recomendado'}
                    </p>
                    {isMCMV && jurosEvolutivos > 0 ? (
                      <p style={{ fontSize: '11px', color: '#78716c' }}>
                        Mensais construtora: {formatBRL(Math.round(mensalValor))}/mês
                        + Juros banco: ~{formatBRL(jurosEvolutivos)}/mês
                        = <strong>{formatBRL(Math.round(burdenMensalTotal))}/mês</strong>
                        · Limite 30%: {formatBRL(Math.round(limite30))}/mês
                      </p>
                    ) : (
                      <p style={{ fontSize: '11px', color: '#78716c' }}>
                        Mensais: {formatBRL(Math.round(mensalValor))}/mês · Limite 30%: {formatBRL(Math.round(limite30))}/mês
                      </p>
                    )}
                  </div>
                </div>
              ...
              <Saldo
                label={`Total pago na obra: ${formatBRL(Math.round(totalNaObra))} — Saldo na entrega`}
                valor={formatBRL(Math.round(saldoNaEntrega))}
            ...
            <BlocoFluxo emoji="🔑" titulo="Na Entrega das Chaves (Habite-se)" cor="#2563eb">
              ...
              <div style={{
                ...
                <p style={{ fontSize: '12px', color: '#78716c', lineHeight: '1.6' }}>
                  {isMCMV
                    ? 'O contrato de financiamento com a Caixa Econômica Federal é assinado 1 a 6 meses após o contrato de compra e venda — antes de a obra começar. Durante a construção, você paga à construtora as mensais + juros evolutivos ao banco (que crescem conforme o progresso da obra).'
                    : 'O financiamento bancário é contratado somente na emissão do Habite-se. Durante toda a obra, você paga apenas à construtora — o banco só entra na entrega das chaves.'}
                </p>
              </div>
            </BlocoFluxo>
            ...
            <BlocoFluxo emoji="📅" titulo="Pós-entrega — Financiamento (35 anos)" cor="#16a34a" ultimo>
              <LinhaFluxo
                label={`Taxa ${isMCMV ? '7,66' : '10,5'}% a.a. · 1ª parcela (SAC — decresce ao longo do tempo)`}
                valor={`${formatBRL(Math.round(parcelaFinanc + seguros.total))}/mês`}
                destaque
              />
            </BlocoFluxo>
          </div>
...
const labelStyle: React.CSSProperties = {
  display: 'block', fontSize: '13px', fontWeight: '600', color: '#1c1917', marginBottom: '4px',
};
const selectStyle: React.CSSProperties = {
  width: '100%', padding: '10px 12px', border: '1.5px solid #e7e5e4',
  borderRadius: '10px', fontSize: '13px', background: '#fff', outline: 'none',
};
...
function PercSelect({ label, hint, valor, onChange, options, base }: {
  ...
  return (
    <div>
      ...
      <select value={valor} onChange={e => onChange(Number(e.target.value))} style={selectStyle}>
        {options.map(o => (
