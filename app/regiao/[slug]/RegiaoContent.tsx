      ...
      <div style={{ padding: '14px 14px 12px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', marginTop: 'auto' }}>
          <Link
            ...
            onClick={e => {
              e.stopPropagation();
              import('@/lib/gtag').then(m => m.trackCtaClick({ origem: 'card_imovel_regiao', destino: '/simulador', texto: 'Simular' }));
            }}
...
// ──────────────────────────────────────────────────────────────────────────────
export default function RegiaoContent({
  region,
  searchParams,
}: {
  region: Region;
  ...
  const [cidade,        setCidade]        = useState(searchParams.cidade || '');
  ...
  const buscar = useCallback(async (p = 1, append = false) => {
    ...
    try {
      const params = new URLSearchParams({
        city:          cidade || region.city,
        neighborhoods: region.neighborhoods.join(','),
        page:          String(p),
        state:         region.state,
      });
      if (!cidade && region.cities && region.cities.length > 0) {
  ...
  }, [region, minPrice, maxPrice, quartos, statusFilter, cidade]);
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <nav style={{
            ...
            <span>›</span>
            <span style={{ color: 'rgba(255,255,255,.7)', fontWeight: '600' }}>{region.name}</span>
          </nav>
          ...
          <div style={{
            ...
            <div>
              <p style={{
                ...
                REGIÃO{region.city ? ` · ${region.city.toUpperCase()}` : ''} · {region.state}
              </p>
              <h1 style={{
                fontSize: 'clamp(28px, 4vw, 42px)', fontWeight: '800',
                color: '#fff', lineHeight: 1.15, marginBottom: '14px',
              }}>
                Imóveis {region.article} {region.name}
              </h1>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '8px' }}>
            <p style={{ fontSize: '14px', color: 'var(--text-muted)' }}>
              {imoveisSorted.length > 0
                ? <><strong style={{ color: 'var(--text)' }}>{imoveisSorted.length}</strong> de <strong style={{ color: 'var(--text)' }}>{total}</strong> {region.article} <strong style={{ color: 'var(--text)' }}>{region.name}</strong></>
                : `Nenhum imóvel ${region.article} ${region.name} com esses filtros`}
            </p>
          </div>
        )}
        ...
        {!loading && imoveisSorted.length === 0 && !erro && (
          <div style={{ textAlign: 'center', padding: '80px 24px' }}>
            ...
            <h2 style={{ fontSize: '20px', fontWeight: '700', color: 'var(--text)', marginBottom: '8px' }}>
              Nenhum imóvel {region.article} {region.name}
            </h2>
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '24px' }}>
              Não encontramos empreendimentos nesta região com os filtros selecionados.
            </p>
        ...
        {/* Bairros desta região — links internos para as páginas de bairro */}
        {!loading && bairrosComImoveis.length > 0 && (
          <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid var(--border)' }}>
            <p style={{ fontSize: '13px', fontWeight: '700', color: 'var(--text-muted)', marginBottom: '12px' }}>
              Bairros da {region.name} com imóveis disponíveis
            </p>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {bairrosComImoveis.map(nb => (
                <Link key={nb} href={bairroPath(nb, region.state)} style={{
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: '260px' }}>
              ...
              <h2 style={{ fontSize: 'clamp(18px, 2.5vw, 24px)', fontWeight: '800', color: '#fff', lineHeight: 1.3, marginBottom: '10px' }}>
                Quanto cabe no seu orçamento<br />{region.article} {region.name}?
              </h2>
              <p style={{ fontSize: '14px', color: 'rgba(255,255,255,.45)', lineHeight: 1.6, maxWidth: '420px' }}>
                ...
                veja o valor das parcelas e compare o financiamento ideal para esta região.
              </p>
            </div>
