...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[api/chat] OpenAI error:', response.status, errText.slice(0, 300));
      if (response.status === 429) {
        return NextResponse.json({ reply: 'Muitas conversas agora. Tente em instantes!' }, { status: 200 });
      }
      throw new Error(`OpenAI ${response.status}`);
    }
    ...
    console.error('[api/chat]', err instanceof Error ? err.message : err);
    return NextResponse.json({ reply: 'Ops, problema tecnico. Tente novamente!' }, { status: 200 });
  }
}
